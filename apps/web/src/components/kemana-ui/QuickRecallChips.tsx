...
export default function QuickRecallChips({ items, onSelect, className }: QuickRecallChipsProps) {
    ...
    return (
        <div className={cn("flex flex-col gap-3", className)}>
            <h3 className="text-[14px] font-bold tracking-tight text-text-primary px-1">
                Catat lagi cepat
            </h3>

            {/* Horizontal Scrollable Row */}
            <div className="-mx-4 flex overflow-x-auto scroll-smooth px-4 pb-4 pt-1 sm:-mx-0 sm:px-0 scrollbar-hide">
                <div className="flex gap-3">
                    {items.map((item) => (
                        <button
                            key={item.id}
                            onClick={() => onSelect?.(item)}
                            className="group flex min-w-[140px] flex-col gap-2.5 rounded-[16px] border border-border-subtle bg-bg-card p-3.5 text-left shadow-sm transition-all hover:border-brand active:scale-[0.98]"
                        >
