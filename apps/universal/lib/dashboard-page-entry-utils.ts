...
import type { TransactionItem } from "@/components/TransactionCard";
...
export function generateCsvString(entries: Entry[]): string {
  const headers = [
    "id",
    "tanggal",
    "kategori",
...
export function downloadCsv(entries: Entry[]): void {
  const content = generateCsvString(entries);
